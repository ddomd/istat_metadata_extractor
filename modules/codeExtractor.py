...
def extractCodes(path):
    ...
    with open(os.path.join(path, "dimensions.json"), "r") as file:
        codelist = json.load(file)  # Load the JSON content into a Python dictionary
    ...
    for key in codelist:
        ...
        response = requests.get(f"https://esploradati.istat.it/SDMXWS/rest/codelist/IT1/{codelist[key]}")
        ...
        with open(os.path.join(path, "dim_json", f"dim_{codelist[key]}.json"), "w" ) as write:
            json.dump( codes , write )

        with open(os.path.join(path, "dim_csv", f"dim_{codelist[key]}.csv"), 'w', encoding="utf-8", newline="") as csv_file:  
